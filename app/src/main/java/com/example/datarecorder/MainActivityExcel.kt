...
private data class FastPackageSummaryItem(
		val packageName: String,
		val angleLengthMeter: Double,
		val templateQuantity: Int,
		val totalAreaRaw: Double
)

private fun MainActivity.calculateFastPackageSummaryItem(
		packageName: String,
		rows: List<FastRow>
): FastPackageSummaryItem {
	var angleLengthMm = 0.0
	var templateQuantity = 0
	var totalAreaRaw = 0.0

	rows.forEach { row ->
		val model = row.model.trim().uppercase(Locale.getDefault())
		val length = row.length.toDoubleOrNull() ?: 0.0
		val qty = calcFastQty(row)

		if (model == "E") {
			angleLengthMm += length * qty
		}

		templateQuantity += qty
		totalAreaRaw += calcFastTotalAreaRaw(row)
	}

	return FastPackageSummaryItem(
		packageName = packageName,
		angleLengthMeter = angleLengthMm / 1000.0,
		templateQuantity = templateQuantity,
		totalAreaRaw = totalAreaRaw
	)
}

private fun MainActivity.buildFastSummaryRows(): List<FastPackageSummaryItem> {
	...
	return packageNames.map { packageName ->
		...
		packageFastRowsMap[packageName]?.let { rows.addAll(it.map { row -> row.copy() }) }
		packageCurrentFastRowMap[packageName]?.let {
			if (!it.isEmpty()) rows.add(it.copy())
		}
		calculateFastPackageSummaryItem(packageName, rows)
	}
}

private fun MainActivity.formatFastSummaryNumber(value: Double): String {
	return if (kotlin.math.abs(value - value.toInt()) < 0.000001) {
...
private fun MainActivity.createFastSummarySheet(
		workbook: XSSFWorkbook,
		sheetName: String,
		summaryRows: List<FastPackageSummaryItem>
) {
	...
	val packageCount = summaryRows.size
	val packageDate = getTodayPackageDate()
	...
	safeMerge(sheet, 0, 1, 0, 0)
	safeMerge(sheet, 0, 0, 1, 5)
	safeMerge(sheet, 1, 1, 1, 5)

	safeMerge(sheet, 2, 2, 1, 2)
	safeMerge(sheet, 2, 2, 4, 5)

	safeMerge(sheet, 3, 3, 1, 2)
	safeMerge(sheet, 3, 3, 4, 5)

	row0.getCell(1).setCellValue("中建铝新材料成都有限公司")
	row0.getCell(1).cellStyle = titleStyle

	row1.getCell(1).setCellValue("返厂面积汇总表")
	row1.getCell(1).cellStyle = subTitleStyle

	row2.getCell(0).setCellValue("项目名称")
	row2.getCell(0).cellStyle = infoHeaderStyle
	row2.getCell(1).setCellValue(projectName)
	row2.getCell(1).cellStyle = infoStyle
	row2.getCell(3).setCellValue("楼栋号：")
	row2.getCell(3).cellStyle = infoHeaderStyle
	row2.getCell(4).setCellValue(buildingName)
	row2.getCell(4).cellStyle = infoStyle

	row3.getCell(0).setCellValue("包数合计")
	row3.getCell(0).cellStyle = infoHeaderStyle
	row3.getCell(1).setCellValue(packageCount.toString())
	row3.getCell(1).cellStyle = infoStyle
	row3.getCell(3).setCellValue("打包日期")
	row3.getCell(3).cellStyle = infoHeaderStyle
	row3.getCell(4).setCellValue(packageDate)
	row3.getCell(4).cellStyle = infoStyle

	row4.getCell(0).setCellValue("序号")
	row4.getCell(1).setCellValue("包号")
	row4.getCell(2).setCellValue("角铝长度（m）")
	row4.getCell(3).setCellValue("模板数量（件）")
	row4.getCell(4).setCellValue("合计面积（㎡）")
	row4.getCell(5).setCellValue("备注")

	var rowIndex = 5
	var totalAngleLength = 0.0
	var totalTemplateQty = 0
	var totalAreaRaw = 0.0

	summaryRows.forEachIndexed { index, item ->
		val row = sheet.createRow(rowIndex).apply { heightInPoints = 24f }

		totalAngleLength += item.angleLengthMeter
		totalTemplateQty += item.templateQuantity
		totalAreaRaw += item.totalAreaRaw
		...
		row.getCell(1).setCellValue(item.packageName)
		row.getCell(2).setCellValue(formatFastSummaryNumber(item.angleLengthMeter))
		row.getCell(3).setCellValue(item.templateQuantity.toString())
		row.getCell(4).setCellValue(areaToSquareMeterText(item.totalAreaRaw))
	...
	totalRow.getCell(2).setCellValue(formatFastSummaryNumber(totalAngleLength))
	totalRow.getCell(3).setCellValue(totalTemplateQty.toString())
	totalRow.getCell(4).setCellValue(areaToSquareMeterText(totalAreaRaw))
	...
	safeMerge(sheet, rowIndex + 1, rowIndex + 1, 0, 2)
	safeMerge(sheet, rowIndex + 1, rowIndex + 1, 3, 5)
	signRow.getCell(0).setCellValue("项目记录人：")
	signRow.getCell(3).setCellValue("中建铝记录人：")
...
fun MainActivity.createFastModeSheet(
	...
	val row2 = sheet.createRow(2).apply { heightInPoints = 24f }
	val row3 = sheet.createRow(3).apply { heightInPoints = 24f }
	val row4 = sheet.createRow(4).apply { heightInPoints = 30f }
	...
	fillRowStyle(row0, 0, 8, infoStyle)
	fillRowStyle(row1, 0, 8, infoStyle)
	fillRowStyle(row2, 0, 8, infoStyle)
	fillRowStyle(row3, 0, 8, infoStyle)
	fillRowStyle(row4, 0, 8, tableHeaderStyle)
	...
	row4.getCell(5).setCellValue("数量（件）")
	...
	rows.forEachIndexed { index, item ->
		...
		fillRowStyle(row, 0, 8, tableCellStyle)
	...
	fillRowStyle(totalRow, 0, 8, totalStyle)
	...
	fillRowStyle(signRow, 0, 8, signStyle)
...
fun MainActivity.buildFastExcelBytes(projectName: String): ByteArray {
	...
	val summaryRows = buildFastSummaryRows()
	createFastSummarySheet(
		workbook = workbook,
		sheetName = "返厂面积汇总",
	...
	workbook.setSheetOrder("返厂面积汇总", 0)
