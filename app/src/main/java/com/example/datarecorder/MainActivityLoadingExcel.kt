...
fun MainActivity.buildLoadingSummaryExcelBytes(projectName: String): ByteArray {
	...
	fun row(index: Int, height: Float = 35f) =
		(sheet.getRow(index) ?: sheet.createRow(index)).apply {
	...
	allItems.forEachIndexed { index, item ->
		...
		setNumber(
			rowIndex,
			7,
			sumIronWeightByMaterial(item.ironRows, "背楞"),
			bodyIntegerStyle,
			blankWhenZero = true
		)
		setNumber(
			rowIndex,
			8,
			sumIronWeightByMaterial(item.ironRows, "吊架"),
			bodyIntegerStyle,
			blankWhenZero = true
		)
...
fun MainActivity.buildLoadingExcelBytes(projectName: String): ByteArray {
	...
	tripNames.forEach { tripName ->
		...
		fun row(index: Int, height: Float = 17f) =
			(sheet.getRow(index) ?: sheet.createRow(index)).apply {
				heightInPoints = height
			}
		...
		val aluminumExcelWeightTotal =
			if (aluminumWeightMode == LoadingWeightMode.WEIGHBRIDGE_TOTAL) {
		...
		aluminumRows.forEach { item ->
			...
			set(
				rowIndex,
				4,
				if (aluminumWeightMode == LoadingWeightMode.WEIGHBRIDGE_TOTAL) "" else item.weight
			)
		...
		ironRows.forEach { item ->
			...
			set(
				rowIndex,
				4,
				if (ironWeightMode == LoadingWeightMode.WEIGHBRIDGE_TOTAL) "" else item.weight
			)
