...
fun MainActivity.showAccountCardPopup(
			...
			username: String,
			onlineText: String,
			connectionCountText: String,
			subDisplayStatusText: String,
			hasSubDisplayCode: Boolean,
			onSubDisplayCodeClick: () -> Unit,
		...
		fun createSubDisplayConnectionRow(popupWindow: PopupWindow): LinearLayout {
			return LinearLayout(this).apply {
				orientation = LinearLayout.HORIZONTAL
				...
				addView(createActionButton(
					...
					popupWindow.dismiss()
					anchor.post { onViewDevicesClick() }
		...
		fun createSubDisplayStatusRow(popupWindow: PopupWindow): LinearLayout {
			val isConnected = subDisplayStatusText == "已连接"
			val statusTextColor = if (isConnected) 0xFF2E8B57.toInt() else 0xFF6B7280.toInt()
			val statusFillColor = if (isConnected) 0xFFEAF8F0.toInt() else 0xFFF1F3F5.toInt()
			val statusStrokeColor = if (isConnected) 0xFFBFE7CF.toInt() else 0xFFD7DCE2.toInt()
			val codeButtonText = if (hasSubDisplayCode) "查看连接码" else "生成连接码"
			val codeButtonBackground = if (hasSubDisplayCode) R.drawable.bg_auth_button_outline else R.drawable.bg_auth_button_primary
			val codeButtonTextColor = if (hasSubDisplayCode) 0xFF6C56B3.toInt() else Color.WHITE
			...
			return LinearLayout(this).apply {
				...
				addView(LinearLayout(this@showAccountCardPopup).apply {
					...
					addView(TextView(this@showAccountCardPopup).apply {
						...
						background = GradientDrawable().apply {
							shape = GradientDrawable.RECTANGLE
							setColor(statusFillColor)
							cornerRadius = dpF(999f)
							setStroke(dp(1), statusStrokeColor)
						}
					}, LinearLayout.LayoutParams(
						LinearLayout.LayoutParams.WRAP_CONTENT,
						LinearLayout.LayoutParams.WRAP_CONTENT
				...
				addView(createActionButton(
					text = codeButtonText,
					backgroundRes = codeButtonBackground,
					textColor = codeButtonTextColor
				) {
					popupWindow.dismiss()
					anchor.post { onSubDisplayCodeClick() }
				}, LinearLayout.LayoutParams(
					LinearLayout.LayoutParams.WRAP_CONTENT,
					dp(38)
				).apply {
		...
		val popupWidth = minOf(dp(300), resources.displayMetrics.widthPixels - dp(24))
		...
		(container.getChildAt(1) as LinearLayout).addView(createSubDisplayConnectionRow(popupWindow))
		(container.getChildAt(1) as LinearLayout).addView(createSubDisplayStatusRow(popupWindow), LinearLayout.LayoutParams(
			LinearLayout.LayoutParams.MATCH_PARENT,
			LinearLayout.LayoutParams.WRAP_CONTENT
		).apply {
			topMargin = dp(8)
